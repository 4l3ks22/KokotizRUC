...
def neighbour_fish_empty_rest(cur,neighbours):
    ...
    """ fish-neighbours, empty-neighbours cells, plants-neighbours cells and the bear-neighbours"""
...
def plant_rules(cur,r,c,neighbour_empty, neighbour_plant):
    """ Given the current grid {cur}, a position (r,c) which contains a plant, and a list of grid-positions for the
    plant-neighbours  and a list of grid-positions of empty neighbour cells. Update the grid according to the plant-rules"""
    ...
    if (len(neighbour_plant) >= plant_overcrowd) or (cur[r, c]['age']==10):
        cur[r, c] = empty()
    return cur
